...
public class MonteCarloTest {
    public static void piDemo() {
        ...
        /*  [monteCarlo]
                (n, experiment) -> n과 experiment라는 매개변수
                Mathx.sum -> Iterator<T> 를 매개변수로 받아 이걸 reduce()를 활용해 반복자의 모든 값을 더한다.
                limit -> n 만큼의 제한만큼 반복자를 생성
                map -> Function(binary)에 맞는 next()를 출력한다.
                generate -> Suppliier(experiment)에게 받는 요소로 무한 반복자를 생성한다.

                experiment에서 공급 받은 끝없는 수열을 binary에 맞게 요소를 순회하면서 n만큼의 제한을 둔다. 해당 값들을 모두 더한뒤 n 으로 나눈다.
         */
        ...
        Function<Supplier<Integer>, Iterator<Double>> monteCarloIterator = new Function<Supplier<Integer>, Iterator<Double>>() {
            @Override
            public Iterator<Double> apply(Supplier<Integer> integerSupplier) {
                return Iterators.zip((sum, count) -> sum / count,
                        Iterators.iterate(0d, x -> x + integerSupplier.get()), Iterators.iterate(1d, x -> x + 1));
            }
        };
...
//        System.out.println(get(pi, 100_1000));
    ...
    private static void potionTestWithInfiniteIterators() {
        ...
        Iterator<Integer> herbAvailablities = Mathx.binaryDistribution(herbRatio); // -> 0.2 보다 큰 rand.double()의 값보다 높으면 1, 낮으면 0을 갖는 무한 반복자
        /*
         ...
         */
        Iterator<Integer> qualities = Mathx.discreteUniformDistribution(Quality.class); // -> 4개의 열거형 중에 랜덤 값 한개의 index를 받아오는것 같음.
        // TODO 아래 주석을 제거하고 첫 번째 인자 채우기
           /*
         Iterator<Quality> herbQualities = zip( (x, y) -> Quality.values()[y],
                                                    Iterators.filter(herbAvailablities, (x) -> x == 1), qualities);
        */

         Iterator<Quality> herbQualities = zip( (x, y) -> x == 1 ? Quality.BEST : Quality.values()[y], herbAvailablities, qualities);
        /*
            1이면 best
         */
        ...
        Iterator<Double> medicineEffects = map(herbQualities, quality -> {
            ...
            if (effect > 100)
                return 100D;
            return effect;
        }); // 기존의 herbQualities를 medicineEffectis(정규 분포가 적용된)로 바꾸는 작업
        /*
            herbQualities -> 랜덤한 허브의 확률을 계산해 qualities를 결정함
            quality -> Function으로 quality에 맞는 key를 가지고 Mathx.randDoubleNormallyDistributed(랜덤 정규 분포)를 받아옴 (뭘 get()하는지는 모르겠음)

            effect -> 0 에서 100 사이의 값 으로만 제한
         */
